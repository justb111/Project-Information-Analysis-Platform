...
4. 请调用 set_jql_filters 函数输出结果，filter_conditions 为 AND 过滤条件。始终获取全量数据，AI后续会根据用户意图从全量数据中筛选和分析。"""
...
请调用 set_jql_filters 函数输出结果。如果不支持函数调用，则直接输出 JQL 过滤条件（只输出 AND 后面的部分，例如：type = Bug AND priority = Blocker）。
"""
...
## ⚠️ 数据完整性声明（重要）：
你接收到的所有Jira数据均为完整的全量数据（包含已解决和未解决），基于全部查询结果的完整计算。你可以从中自行计算未解决率、解决率、闭环情况等任何指标。
**严禁**使用"样本"、"抽样"、"基于可见数据"、"前N个"等暗示数据被截断的说法。
...
class RiskAnalysisAgent:
    ...
    def generate_jql(self):
        ...
        # ── 优先用 Function Calling（兼容不支持强制 tool_choice 的 API）──
        result = _call_llm_with_tools(
        ...
        # ── 始终全量获取，不分 resolved/unresolved ──
        self.jql_all = jql

        return True

    def fetch_data(self):
        """Step 3: Fetch ALL Jira data (no artificial limit, no resolved/unresolved split)"""
        if self.jql_all:
    ...
    def stream_analysis(self, sse_queue, user_query, conversation_history=None):
        ...
        if is_portfolio and self.issues_all:
    ...
    def _generate_kanban_data(self):
        ...
        return {
            ...
            "unresolved": len(columns['high_risk']) + len(columns['medium_risk']) + len(columns['low_risk'])
        }
    ...
    def _run_jira_pipeline(self, user_query, sse_queue, cancel_event, conversation_history):
        ...
        # Step 3: Fetch data
        sse_queue.put(('thinking', '📡 正在从Jira获取全量数据...'))
        self.fetch_data()
        if cancel_event and cancel_event.is_set():
            sse_queue.put(('done', '分析已取消'))
            return
        ...
        sse_queue.put(('thinking', f'📊 获取到 {total_all} 条问题'))
