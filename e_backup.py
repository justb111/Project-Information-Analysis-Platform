...
def stream_portfolio_analysis(issues_all, enhanced_query, sse_queue, system_prompt, max_tokens=16384, project_names=None):
    """
    全量项目群风险分析 - 统一入口

    接收所有Jira问题（全量，不分resolved/unresolved），进行全量统计和智能格式化，
    然后流式调用AI进行分析，将结果推送到SSE队列。
    AI会根据用户意图自动从全量数据中筛选和分类。
